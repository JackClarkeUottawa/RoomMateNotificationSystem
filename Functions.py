...
def rearrange_data(pat):
    ...
    if os.path.exists(pat):
        a = readfromfile(pat)
        b = shift(a)
        writeArraytofile(b,"schedule.json")
    else:
...
def mainthing():
    ...
    if date.today().isoweekday() == 5: # day is firday
        rearrange_data("schedule.json")# file needs to be changed for server, maybe
        a = readfromfile("schedule.json")
        for num in range(len(a)):
            if a[num] not in ["Garbage 1","Garbage 2"]:
        ...
        for num in range(len(a)):
            if a[num] == "Garbage 1":
                ...
                Gp2 = num
        msg1 = "Hi "+users[Gp1]["Name"] +" This week you taking out the trash  "+users[Gp2]["Name"]
        msg2 = "Hi "+users[Gp2]["Name"] +" you're taking out the trassh "+users[Gp1]["Name"]
